---
title: "Teaching"
permalink: /teaching/
layout: archive
author_profile: true
---

I fairly enjoy teaching as it often provides a much needed opportunity to connect fundamental concepts with modern astronomical research. My usual goal through the courses is to help students develop an interest in the subject matter, and alongside the intuition and skillsets needed to tackle real scientific problems-- which unlike classroom problems do not have a solution.

I will add Teaching material, lecture notes, and additional resources here as they become available.

---

## Masters Course in Astronomical Instrumentation at University of Cape Town (2024)

I co-developed and taught the inaugural Master's-level course in Astronomical Instrumentation at the University of Cape Town. The course introduced the principles behind modern astronomical instruments, covering topics from telescope optics and detectors to spectroscopy, polarimetry, calibration, and instrument design.

### Lecture Notes

- 📄 [Lecture 1: Introduction to Astronomical Polarimetry]({{ '/files/Introduction_to_Polarimetry_Lecture.pdf' | relative_url }})

- 📄 [Lecture 2: Errors and Statistics]({{ '/files/Introduction_to_Errors.pdf' | relative_url }})
